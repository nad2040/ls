...
typedef enum dots_opt {
	...
	ALL_DOTS	/* -a flag - show dotfiles and dotdirs and default - cannot be changed once set */
} dots_opt;
...
typedef enum sort_opt {
	...
	TIME_SORT,	/* -t flag - first by the specified time_opt. then by ascii */
	SIZE_SORT	/* -S flag - first by the size. then by ascii */
} sort_opt;

typedef enum blkcount_fmt_opt {
	BLKSIZE_ENV,	/* -s flag default behavior - number of (getenv(BLOCKSIZE) or 512)-byte blocks */
	BLKSIZE_KILO,	/* -k flag - number of kilobyte blocks (1024) */
	HUMAN_READABLE	/* -h flag - num of blocks (rounded) get's converted to a human readable number. also impacts printing of regular size */
} blkcount_fmt_opt;
